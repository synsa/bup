...
def summary_bytes(meta, numeric_ids = False, classification = None,
    ...
    if meta:
        ...
        mtime_str = strftime('%Y-%m-%d %H:%M',
                             time.localtime(mtime_secs)).encode()
...
def detailed_bytes(meta, fields = None):
    ...
    if 'mode' in fields:
        result.append(b'mode: %s (%s)' % (oct(meta.mode).encode('ascii'),
                                          xstat.mode_str(meta.mode).encode('ascii')))
    ...
    if 'size' in fields and meta.size is not None:
        result.append(b'size: ' + str(meta.size).encode('ascii'))
    if 'uid' in fields:
        result.append(b'uid: ' + str(meta.uid).encode('ascii'))
    if 'gid' in fields:
        result.append(b'gid: ' + str(meta.gid).encode('ascii'))
    if 'user' in fields:
    ...
    if 'atime' in fields:
        ...
        if xstat.lutime or not stat.S_ISLNK(meta.mode):
            result.append(b'atime: ' + xstat.fstime_to_sec_str(meta.atime).encode('ascii'))
        else:
            result.append(b'atime: 0')
    if 'mtime' in fields:
        if xstat.lutime or not stat.S_ISLNK(meta.mode):
            result.append(b'mtime: ' + xstat.fstime_to_sec_str(meta.mtime).encode('ascii'))
        else:
            result.append(b'mtime: 0')
    if 'ctime' in fields:
        result.append(b'ctime: ' + xstat.fstime_to_sec_str(meta.ctime).encode('ascii'))
